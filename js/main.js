...
var resizePizzas = function(size) {
  ...
  // Change all querySelector to getElementById for performance improvement.
  ...
  //Instead of calculating newWidth with value of dx and also use determineDx and offsetWidth which takes up long time, 
  //just used switch to assign the pizza size to newWidth and change the slider value to percent width.
  var newWidth;
    switch(size) {
      case "1":
        newWidth = 25;
...
// Move document.querySelectorAll(".randomPizzaContainer") outside the for loop.
// Switch to document.getElementsByClassName from document.querySelectorAll.
    var container = document.getElementsByClassName("randomPizzaContainer");

// Move random pizza container length to a local variable contLength so the array's length property isn't accessed on each iteration
...
// moved pizzasDiv variable outside the loop to call DOM only once.
...
//For better performance changed document.querySelector to document.getElementsByClassName
//Assign the value of scrollTop to variable scrollUp outside the loop to avoid carrying out layout multiple times.
//Declared the variable phase outside the loop.Thus moved all the constants out of the FOR loop.
//store values from for loop in pizzaArray.
// Used transform property for visual formatting of pizza and also used translateX() to move the pizzas horizontally on the plane bcoz basic.left property takes large time to paint each frame.
//Inorder to make it work made some changes in .mover class in CSS too.
 // window.items = document.getElementsByClassName('mover');
  var scrollUp = (document.body.scrollTop/ 1250);
  var pizzaArray =[];

  for(var i = 0; i < 5; i++){
    pizzaArray.push(Math.sin(scrollUp + i));
  }
  var phase;
  for( i=0; i < window.items.length; i++){
    phase = pizzaArray[ i%5];
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  //calculate number of background pizza on the basis of viewport size;
  var screenHeight = window.innerHeight;
  var rows = (Math.ceil(screenHeight/s) + 1);
  console.log(rows);
  ...
  //Moved the below statement into this function inorder stop updatePositions from re-defining items on every scroll event.
